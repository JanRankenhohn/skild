...
type PersistedInstalledDependency = Omit<InstalledDependency, "installDir"> & {
  installDir?: string;
};

type PersistedInstallRecord = Omit<
  InstallRecord,
  "installDir" | "installedDependencies"
> & {
  installDir?: string;
  installedDependencies?: PersistedInstalledDependency[];
};

function toPersistedInstallRecord(
...
function hydrateInstallRecord(
  installDir: string,
  record: PersistedInstallRecord,
): InstallRecord {
  const hydrated: InstallRecord = { ...record, installDir } as InstallRecord;
  if (hydrated.installedDependencies?.length) {
    hydrated.installedDependencies = hydrated.installedDependencies.map(
      (dep) => {
        if (dep.sourceType === "inline") return dep;
        return {
          ...dep,
          installDir: getSkillInstallDir(
            hydrated.platform,
            hydrated.scope,
            dep.name,
          ),
...
function sanitizeLockfile(lockfile: Lockfile): Lockfile {
  const entries = Object.fromEntries(
    Object.entries(lockfile.entries).map(([name, entry]) => [
      name,
      toPersistedLockEntry(entry),
    ]),
  );
...
function applyGlobalConfigDefaults(
...
// ============================================================================
// Prompt Install Records
// ============================================================================

export function writePromptRecord(
  platform: Platform,
  scope: InstallScope,
  record: PromptInstallRecord,
): void {
...
export function readPromptRecord(
  ...
  scope: InstallScope,
  name: string,
): PromptInstallRecord | null {
  const filePath = getPromptInstallRecordPath(platform, scope, name);
  return readJsonFile<PromptInstallRecord>(filePath);
}

export function removePromptRecord(
  platform: Platform,
  scope: InstallScope,
  name: string,
): void {
