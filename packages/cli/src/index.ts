...
program
  ...
  .option(
    "--recursive",
    "If source is a multi-skill directory/repo, install all discovered skills",
  )
  .option(
    "--skill <path-or-name>",
    "Select a single skill when the source contains multiple skills",
  )
  .option("-y, --yes", "Skip confirmation prompts (assume yes)")
  .option("--depth <n>", "Max markdown recursion depth (default: 0)", "0")
  .option(
    "--scan-depth <n>",
    "Max directory depth to scan for SKILL.md (default: 6)",
    "6",
  )
  .option(
    "--max-skills <n>",
    "Max discovered skills to install (default: 200)",
    "200",
  )
  ...
  .option(
    "--registry <url>",
    "Registry base URL (default: https://registry.skild.sh)",
  )
  .option("--json", "Output JSON")
  .option("--type <type>", "Artifact type: skill, prompt (default: skill)")
...
program
  .command("signup")
  .description(
    "Create a publisher account in the registry (no GitHub required)",
  )
  .option(
    "--registry <url>",
    "Registry base URL (default: https://registry.skild.sh)",
  ...
  .option(
    "--handle <handle>",
    "Publisher handle (owns @handle/* scope) (optional; will prompt)",
  )
...
program
  ...
  .option(
    "--handle-or-email <value>",
    "Handle or email (optional; will prompt)",
  )
  .option("--password <password>", "Password (optional; will prompt)")
...
program
  ...
  .option(
    ...
    "Override skill name (defaults to SKILL.md frontmatter)",
  )
  // NOTE: Do not use `--version` here; it conflicts with Commander’s built-in `--version`.
  .option(
    "--skill-version <semver>",
    "Override version (defaults to SKILL.md frontmatter)",
  )
  .option(
    "--alias <alias>",
    "Optional short identifier (global unique) for `skild install <alias>`",
  )
  .option(
    "--description <text>",
    "Override description (defaults to SKILL.md frontmatter)",
  )
  .option(
    "--targets <list>",
    "Comma-separated target platforms metadata (optional)",
  )
...
program
  ...
  .option(
    "--path <path>",
    "Target path inside repo (defaults to skills/<skill-name>)",
  )
  ...
  .action(async (repo: string | undefined, options: any) =>
    push(repo, options),
  );
...
program
  ...
  .option(
    "--max-skills <n>",
    "Max discovered skills to export (default: 200)",
    "200",
  )
  .option("--json", "Output JSON")
  .action(async (source: string, options: any) => {
...
program
  .command("sync [skills...]")
  .description(
    "Sync missing skills across platforms (auto-detect + tree selection)",
  )
  .option(
    "--to <platforms>",
    `Comma-separated target platforms to include: ${PLATFORMS.join(", ")}`,
  )
  .option("--all", "Alias for default (all platforms)")
  .option("-l, --local", "Use project-level directory instead of global")
  .option(
    "-y, --yes",
    "Skip interactive prompts (defaults to all other platforms)",
  )
...
program.action(() => {
  ...
  console.log(
    `    ${dim("$")} ${cyan("skild install anthropics/skills")}   ${dim("Install Anthropic's official skills")}`,
  );
  console.log(
    `    ${dim("$")} ${cyan("skild list")}                        ${dim("View installed skills")}`,
  );
  console.log(
    `    ${dim("$")} ${cyan("skild sync")}                        ${dim("Sync skills across platforms")}`,
  );
  console.log("");
  ...
  console.log(
    `    ${green("install")}   ${dim("i")}    Install skills from GitHub, registry, or local`,
  );
  console.log(
    `    ${green("list")}      ${dim("ls")}   List all installed skills`,
  );
  console.log(
    `    ${green("info")}            Show details about an installed skill`,
  );
  console.log(`    ${green("uninstall")} ${dim("rm")}   Uninstall a skill`);
  console.log(
    `    ${green("update")}    ${dim("up")}   Update installed skills`,
  );
  ...
  console.log(
    `    ${green("validate")}  ${dim("v")}    Validate a skill folder`,
  );
  console.log(
    `    ${green("publish")}         Publish a skill to the registry`,
  );
  console.log("");
